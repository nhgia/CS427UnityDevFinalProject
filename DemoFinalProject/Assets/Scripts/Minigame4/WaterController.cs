...
public class WaterController : MonoBehaviour {
	...
	List<RippleSource> waveSources;	//all of the wave sources (will be collected by tag)
	// declaring variables
	float waterLevelY;			// the water - surface's level (along the Y - axis)
	public float surfaceActualWidth;	// surface - dimension along the X-axis
	public float surfaceActualLength;	// surface - dimension along the Z-axis
	public int surfaceWidthPoints;		// number of points on the X-axis
	public int surfaceLengthPoints;	// number of points on the Z-axis
	...
	void Awake () {
		...
		waterLevelY = 0.0f;
		/*surfaceActualWidth = 15;
		surfaceActualLength = 15;
		surfaceWidthPoints = 150;
		surfaceLengthPoints = 150;*/
		localTime = 0.0f;
		localTimeScale = 2.0f;

		CreateMesh ();	// create the initial geometry of the water-mesh

		waveSources = new List<RippleSource> ();										// initialize the wave sources list
		//waveSources = FindObjectsOfType<RippleSource>().ToList ();	//find all the wavesources in the scene and pass them in the list
	}

	// Update is called once per frame
	void Update () {

        waveSources = FindObjectsOfType<RippleSource>().ToList();
	...
	private float RecalculatePointY(Vector3 point){
		...
		for (int i = 0; i < waveSources.Count; i++) {	//cycle through all of the wave sources
            ...
            RippleSource rippleSource = waveSources[i];
            float distance = Vector3.Distance(point, waveSources[i].transform.position);

            if (rippleSource.stop && distance < rippleSource.stopDistance) continue; 
            if (distance > rippleSource.maxDistance) continue;
			...
			y += Mathf.Sin (dist * 12.0f - localTime * rippleSource.speed) / (dist*20.0f+10.0f);	// apply the first wave
		}
